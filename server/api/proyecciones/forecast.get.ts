...
export default defineEventHandler(async (event) => {
  ...
  const [
    ...
    prisma.expense.findMany({
      where: { userId: user.id, creditCardId: { not: null }, date: { gte: threeMonthsAgo, lte: now } },
    }),
  ])

  // Current balance = income received this month - cash expenses paid this month
  const currentBalance =
    thisMonthIncome.reduce((s, i) => s + i.amount, 0) -
    thisMonthCashExpenses.reduce((s, e) => s + e.amount, 0)

  const monthlyRecurringIncome = recurringIncomes.reduce((s, i) => s + i.amount, 0)
  const monthlyRecurringExpenses = recurringExpenses.reduce((s, e) => s + e.amount, 0)
  const monthlyCardEstimate = recentCCExpenses.reduce((s, e) => s + e.amount, 0) / 3
  ...
  for (const inst of futureInstallments) {
    ...
    installmentsByMonth.set(key, (installmentsByMonth.get(key) ?? 0) + inst.amount)
  }

  // Determine last installment per debt (for "freed" calculation)
  const debtLastMonth = new Map<number, { name: string; lastDueDate: Date; monthlyPayment: number }>()
  for (const inst of futureInstallments) {
    ...
    if (!existing || dueDate > existing.lastDueDate) {
      debtLastMonth.set(inst.debtId, {
        name: inst.debt.name,
        lastDueDate: dueDate,
        monthlyPayment: inst.debt.monthlyPayment,
      })
    }
  }

  const debtsFreed = Array.from(debtLastMonth.values()).map((info) => {
