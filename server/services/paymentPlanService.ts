...
function estimateNextIncomeDate(
  ...
  const ref =
    recurringIncomes.find((i) => i.frequency === 'monthly' || i.frequency === null) ??
    recurringIncomes[0]

  const dayOfMonth = ref ? new Date(ref.date).getDate() : 25
  const candidate = new Date(now.getFullYear(), now.getMonth(), dayOfMonth)
  return candidate > now ? candidate : new Date(now.getFullYear(), now.getMonth() + 1, dayOfMonth)
}
...
export async function getPaymentSuggestions(userId: number) {
  ...
  const [
    ...
    // Saldo real de efectivo/débito (ingresos - gastos cash/débito - cuotas de deuda
    // pagadas este mes); ver server/utils/cash-flow.ts. Compartido con
    // budgets/calculate.post.ts para no duplicar esta lógica ni desalinearla.
  ...
  // income.amount viene de la BD como Prisma.Decimal; Number(...) lo normaliza
  // para poder sumar con aritmética JS (`+` concatena strings si se deja un
  // Decimal sin convertir).
  const totalReceivedIncome = receivedIncomesThisMonth.reduce((sum, i) => sum + Number(i.amount), 0)

  const totalRecurringIncome = recurringIncomes.reduce((sum, i) => sum + Number(i.amount), 0)
  ...
  for (const debt of activeDebts) {
    ...
    suggestions.push({
      ...
      // nextInstallment.amount / debt.remainingAmount vienen como Prisma.Decimal
  ...
  const cardStatements = await Promise.all(
    creditCards.map(async (card) => {
      ...
      // e.amount / e.installmentAmount vienen como Prisma.Decimal; Number(...) los
      // normaliza antes de sumar.
  ...
  if (cashFlowStatus === 'deficit') {
    warnings.push(
      `🚨 ALERTA: Te faltan ${formatUSD(Math.abs(availableAfterObligations))} para cubrir todas tus obligaciones.`
    )
  }
  if (pendingRecurringIncome > 0) {
    warnings.push(
      `💰 Tienes ${formatUSD(pendingRecurringIncome)} de ingresos pendientes por recibir este mes.`
    )
  }
  ...
  if (currentBalance < safetyBuffer && cashFlowStatus !== 'deficit') {
    warnings.push(
      `💡 Tu saldo actual está por debajo del colchón recomendado (${formatUSD(safetyBuffer)}).`
    )
  }
  ...
  const highInterestDebts = suggestions.filter(
    (s) => s.type === 'debt' && (s.interestRate ?? 0) > 20
  )
