...
export function buildInstallments(params: InstallmentInput) {
  const { debtId, totalAmount, monthlyPayment, interestRate, totalInstallments, startDate, paymentDayOfMonth } = params
...
export async function processDebtPayment(params: {
  ...
  const { debtId, userId, amount, principal, interest, insurance = 0, date, paymentNumber, notes, installmentIds } = params
  ...
  const payment = await prisma.debtPayment.create({
    data: { amount, principal, interest, insurance, date: date ?? new Date(), paymentNumber, notes, debtId },
  })

  const now = new Date()
  const idsToMark = installmentIds?.length ? installmentIds : debt.installments[0] ? [debt.installments[0].id] : []
  ...
  const newRemaining = Math.max(0, debt.remainingAmount - principal)
...
export async function updateDebtPayment(params: {
  ...
  const { paymentId, userId, amount, principal, interest, insurance, date, paymentNumber, notes } = params
  ...
  const principalDelta = principal - payment.principal
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const newRemaining = Math.max(0, payment.debt.remainingAmount - principalDelta)
...
export async function reverseDebtPayment(params: { paymentId: number; userId: number }) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const newRemaining = payment.debt.remainingAmount + payment.principal
    await tx.debt.update({
      where: { id: payment.debtId },
