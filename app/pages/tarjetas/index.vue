...
const { data: creditCards, pending, error, refresh } = await useFetchAuth<CreditCard[]>('/api/credit-cards')
  ...
  try {
    await Promise.all(
      activeCards.map(async (card) => {
        try {
          const data = await $fetch<StatementResponse>(`/api/credit-cards/${card.id}/statement`)
...
const openCreateModal = () => { editingCard.value = null; showFormModal.value = true }
const openEditModal = (card: CreditCard) => { editingCard.value = card; showFormModal.value = true }
const openPaymentModal = (card: CreditCard, amount: number) => { cardToPay.value = { card, suggestedAmount: amount }; showPaymentModal.value = true }
const openPaymentHistoryModal = (card: CreditCard) => { cardForHistory.value = { id: card.id, name: card.name }; showPaymentHistoryModal.value = true }

const handleSave = async () => { await refresh(); await loadStatements() }
const handlePaymentSave = async () => { await refresh(); await loadStatements() }
...
const deleteCard = async (card: CreditCard) => {
  ...
  try {
    ...
    await refresh(); await loadStatements()
  } catch {
  ...
  <div class="mx-auto max-w-5xl space-y-5">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500">{{ activeCards.length }} tarjeta{{ activeCards.length !== 1 ? 's' : '' }} activa{{ activeCards.length !== 1 ? 's' : '' }}</p>
      </div>
      <button
        ...
        >
          ...
          >
            ...
            <div class="relative flex items-start justify-between">
              <div>
                <p class="text-[11px] font-medium uppercase tracking-widest text-white/60">{{ card.bank }}</p>
                <p class="text-lg font-bold text-white leading-tight">{{ card.name }}</p>
              </div>
          ...
          <div class="p-4">
            ...
            <div v-else-if="getStatement(card.id)" class="space-y-4">
              ...
              <div class="flex items-start justify-between">
                <div>
                  <p class="text-xs font-medium text-gray-400 uppercase tracking-wide">Deuda del periodo</p>
                ...
                <div class="text-right">
                  ...
                  <p class="text-sm font-semibold text-gray-700">
                    {{ formatDate(getStatement(card.id).billingPeriod?.paymentDueDate ?? getStatement(card.id).paymentDueDate) }}
                  </p>
                  <p
                    class="text-xs"
                    :class="urgencyClass(daysUntilDue(getStatement(card.id)))"
                  >
                    {{ daysUntilDue(getStatement(card.id)) !== null ? `${daysUntilDue(getStatement(card.id))} días` : '' }}
                  </p>
                </div>
              </div>
              ...
              <div>
                <div class="mb-1 flex justify-between text-xs text-gray-500">
                  ...
                  <span class="font-medium">{{ (getStatement(card.id).creditUsagePercent ?? 0).toFixed(1) }}%</span>
                </div>
                ...
                <div class="mt-1 flex justify-between text-xs text-gray-400">
                  <span>Disponible: {{ formatCurrency(getStatement(card.id).availableCredit ?? 0) }}</span>
              ...
              <div class="flex gap-2">
                <span class="flex items-center gap-1 rounded-lg bg-gray-50 px-2.5 py-1.5 text-xs font-medium text-gray-600 ring-1 ring-gray-200">
        ...
        <div class="divide-y divide-gray-50">
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                ...
                <span
                  ...
                  :class="card.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'"
                >
            ...
            <div class="flex shrink-0 gap-1 opacity-0 transition-opacity group-hover:opacity-100">
              <button @click="openEditModal(card)" class="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-indigo-50 hover:text-indigo-600" title="Editar">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                </svg>
              </button>
              <button @click="deleteCard(card)" class="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-500" title="Eliminar">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                </svg>
              </button>
    ...
    <!-- Modales -->
    <TarjetasCreditCardFormModal v-model:show="showFormModal" :card="editingCard" @save="handleSave" />
