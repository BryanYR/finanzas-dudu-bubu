...
const totalPending = computed(() =>
  activeDebts.value.reduce((s, d) => s + d.remainingAmount, 0),
)
const totalOriginal = computed(() =>
  debts.value.reduce((s, d) => s + d.totalAmount, 0),
)
const totalPaid = computed(() =>
  debts.value.reduce((s, d) => s + (d.totalAmount - d.remainingAmount), 0),
)
const monthlyTotal = computed(() =>
  activeDebts.value.reduce((s, d) => s + d.monthlyPayment, 0),
)
...
// Actions
const handleCreate = () => { editingDebt.value = null; showDebtModal.value = true }
const handleEdit = (d: Debt) => { editingDebt.value = d; showDebtModal.value = true }
...
const handleHistory = (d: Debt) => { selectedDebt.value = d; showHistoryModal.value = true }

const handleInstallments = (d: Debt) => {
  selectedDebt.value = d; showInstallmentsModal.value = true
}
...
<template>
  <div class="mx-auto max-w-5xl space-y-5">
    ...
    <div v-if="loading" class="flex justify-center py-16">
      <div class="h-8 w-8 animate-spin rounded-full border-[3px] border-purple-600 border-t-transparent"></div>
    </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-3 gap-3">
        <div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100">
          ...
          <p class="mt-1.5 text-lg font-bold text-red-500 lg:text-xl">{{ formatCurrency(totalPending) }}</p>
          <p class="mt-0.5 text-xs text-gray-400">{{ activeDebts.length }} préstamo{{ activeDebts.length !== 1 ? 's' : '' }}</p>
        </div>
        <div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100">
          <p class="text-xs font-medium uppercase tracking-wide text-gray-400">Cuota mensual</p>
          <p class="mt-1.5 text-lg font-bold text-orange-500 lg:text-xl">{{ formatCurrency(monthlyTotal) }}</p>
        ...
        <div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100">
          ...
          <p class="mt-1.5 text-lg font-bold text-emerald-600 lg:text-xl">{{ formatCurrency(totalPaid) }}</p>
      ...
      <div class="flex gap-2">
        ...
        >
          ...
          >
            {{ f.val === 'all' ? debts.length : f.val === 'active' ? activeDebts.length : paidDebts.length }}
          </span>
        </button>
      </div>
      ...
      <div class="space-y-3">
        ...
        >
          ...
          <div
            class="h-1 w-full"
            :class="debt.isPaid ? 'bg-emerald-400' : 'bg-purple-500'"
          />
          ...
          <div class="p-4 lg:p-5">
            ...
            <div class="flex items-start justify-between gap-3">
              ...
              <div class="shrink-0 text-right">
                <p class="text-lg font-bold text-gray-900">{{ formatCurrency(debt.monthlyPayment) }}</p>
            ...
            <div class="mt-3 flex flex-wrap gap-4">
              <div>
                ...
                <p class="text-sm font-bold" :class="debt.isPaid ? 'text-emerald-600' : 'text-red-500'">
                  {{ formatCurrency(debt.remainingAmount) }}
                </p>
              </div>
              <div>
                <p class="text-xs text-gray-400">Original</p>
                <p class="text-sm font-semibold text-gray-700">{{ formatCurrency(debt.totalAmount) }}</p>
              </div>
              <div>
            ...
            <div class="mt-4">
              <div class="mb-1.5 flex items-center justify-between text-xs text-gray-500">
                ...
                <span class="font-semibold" :class="debt.isPaid ? 'text-emerald-600' : 'text-gray-700'">
                  {{ getProgress(debt) }}%
                </span>
              </div>
    ...
    <DeudasDebtHistoryModal v-model:show="showHistoryModal" :debt="selectedDebt" @deleted="handleSave" />
