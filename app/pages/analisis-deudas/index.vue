...
import type { AmortizationTable, SimulatorResult, CashForecast } from '#types/debt-analysis'
...
} = await useFetchAuth<CashForecast>(
  () => `/api/analisis-deudas/forecast?months=${forecastMonths.value}`
)
...
const loadAmortization = async () => {
  ...
  try {
    amortData.value = await $authFetch<AmortizationTable>(
      `/api/analisis-deudas/amortizacion/${selectedAmortDebtId.value}`
    )
  } finally {
...
const runSimulator = async () => {
  ...
  simError.value = null
  try {
    simData.value = await $authFetch<SimulatorResult>(
      `/api/analisis-deudas/simulador?debtId=${selectedSimDebtId.value}&extraPayment=${extraPayment.value}`
    )
  } catch (err: any) {
...
<template>
  <div class="mx-auto max-w-6xl space-y-5">
    <!-- Header -->
    <div>
      <h1 class="text-xl font-bold text-gray-900 lg:text-2xl">Análisis de Deudas</h1>
      <p class="text-sm text-gray-500">
        Simula escenarios, visualiza tu amortización y proyecta tu flujo
      </p>
    </div>
    ...
    <div v-if="activeTab === 'simulador'" class="space-y-5">
      ...
      <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
        <p class="mb-4 text-sm font-semibold text-gray-700">¿Cuánto puedes pagar de más hoy?</p>
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <div v-if="selectedSimDebt" class="mt-2 flex flex-wrap gap-1.5">
              <button
                v-for="opt in [
                  { label: '+1 cuota', mult: 1 },
                  { label: '+2 cuotas', mult: 2 },
                  { label: '+3 cuotas', mult: 3 },
                ]"
                :key="opt.label"
        ...
        <!-- Debt info mini card -->
        <div
          v-if="selectedSimDebt"
          class="mt-4 flex flex-wrap gap-4 rounded-xl bg-gray-50 p-3 text-xs text-gray-600"
        >
          <span
            >Saldo:
            <strong class="text-gray-900">{{
              formatCurrency(selectedSimDebt.remainingAmount)
            }}</strong></span
          >
          <span
            >Cuota mensual:
            <strong class="text-gray-900">{{
              formatCurrency(selectedSimDebt.monthlyPayment)
            }}</strong></span
          >
          <span
            >TEA: <strong class="text-rose-600">{{ selectedSimDebt.interestRate }}%</strong></span
          >
          <span
            >Cuotas restantes:
            <strong class="text-gray-900">{{
              selectedSimDebt._count?.installments ?? '—'
            }}</strong></span
          >
        </div>
      ...
      <div
        v-if="simError"
        class="rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700 ring-1 ring-red-200"
      >
        {{ simError }}
      </div>
      ...
      <div v-if="simData" class="space-y-4">
        <!-- Header result -->
        <div
          class="rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-700 p-5 text-white shadow-lg"
        >
          <p class="text-xs font-semibold uppercase tracking-widest text-indigo-200">
            Simulación · {{ simData.debt.name }}
          </p>
        ...
        <div class="grid gap-4 lg:grid-cols-2">
          ...
          <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
            <div class="bg-emerald-600 px-4 py-3 text-white">
              <p class="text-[11px] font-bold uppercase tracking-widest text-emerald-100">
                Opción A
              </p>
            ...
            <div class="space-y-3 p-4">
              <div class="flex items-center justify-between">
                ...
                <div class="text-right">
                  <span class="text-sm text-gray-400 line-through">{{
                    simData.debt.remainingInstallments
                  }}</span>
                  <span class="ml-2 text-base font-bold text-emerald-700">{{
                    simData.reduccionPlazo.newInstallments
                  }}</span>
                  <span
                    class="ml-1 rounded-full bg-emerald-100 px-1.5 py-0.5 text-[10px] font-bold text-emerald-700"
                  >
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-sm font-semibold text-gray-700"
                  >{{ formatCurrency(simData.debt.monthlyPayment) }} (sin cambio)</span
                >
              </div>
              <div class="flex items-center justify-between">
                <span class="text-xs text-gray-500">Nueva fecha fin</span>
                <span class="text-sm font-semibold text-emerald-700">{{
                  formatDate(simData.reduccionPlazo.completionDate)
                }}</span>
              </div>
              <div class="rounded-xl bg-emerald-50 p-3">
                ...
                <div class="mt-1 flex items-baseline gap-2">
                  <span class="text-xs text-gray-400 line-through">{{
                    formatCurrency(simData.reduccionPlazo.totalInterestNow)
                  }}</span>
                  <span class="text-base font-bold text-emerald-800">{{
                    formatCurrency(simData.reduccionPlazo.totalInterestAfter)
                  }}</span>
                </div>
          ...
          <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
            ...
            <div class="space-y-3 p-4">
              <div class="flex items-center justify-between">
                <span class="text-xs text-gray-500">Cuotas restantes</span>
                <span class="text-sm font-semibold text-gray-700"
                  >{{ simData.reduccionCuota.remainingInstallments }} (sin cambio)</span
                >
              </div>
              <div class="flex items-center justify-between">
                ...
                <div class="text-right">
                  <span class="text-sm text-gray-400 line-through">{{
                    formatCurrency(simData.debt.monthlyPayment)
                  }}</span>
                  <span class="ml-2 text-base font-bold text-blue-700">{{
                    formatCurrency(simData.reduccionCuota.newMonthlyPayment)
                  }}</span>
                  <span
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-sm font-semibold text-gray-700"
                  >{{ formatDate(simData.reduccionCuota.completionDate) }} (sin cambio)</span
                >
              </div>
              <div class="rounded-xl bg-blue-50 p-3">
                ...
                <div class="mt-1 flex items-baseline gap-2">
                  <span class="text-xs text-gray-400 line-through">{{
                    formatCurrency(simData.reduccionCuota.totalInterestNow)
                  }}</span>
                  <span class="text-base font-bold text-blue-800">{{
                    formatCurrency(simData.reduccionCuota.totalInterestAfter)
                  }}</span>
                </div>
        ...
        <div class="rounded-2xl border border-amber-200 bg-amber-50 p-4">
          ...
          <p class="mt-1 text-sm text-amber-800">
            <template
              v-if="simData.reduccionPlazo.interestSaved >= simData.reduccionCuota.interestSaved"
            >
              <strong>Elige Reducción de Plazo (Opción A)</strong> si tu banco lo permite. Ahorras
              <strong
                >{{
                  formatCurrency(
                    simData.reduccionPlazo.interestSaved - simData.reduccionCuota.interestSaved
                  )
                }}
                más</strong
              >
              en intereses que la opción B y terminas
              {{ simData.reduccionPlazo.installmentsSaved }} cuotas antes.
            </template>
            <template v-else>
              <strong>Elige Reducción de Cuota (Opción B)</strong> si necesitas aliviar tu flujo
              mensual. Reduces tu cuota en
              <strong>{{ formatCurrency(simData.reduccionCuota.monthlySavings) }}/mes</strong>.
            </template>
      ...
      >
        ...
        <p class="mt-4 text-sm font-medium text-gray-600">
          Selecciona una deuda e ingresa el monto a pagar de más
        </p>
        <p class="mt-1 text-xs text-gray-400">
          El simulador te mostrará cuánto ahorras según cada estrategia
        </p>
      </div>
    </div>
    ...
    <div v-else-if="activeTab === 'amortizacion'" class="space-y-5">
      <!-- Controls -->
      <div
        class="flex flex-wrap items-end gap-4 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100"
      >
        ...
        <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
          <div class="border-b border-gray-50 px-4 py-3">
            <p class="text-sm font-semibold text-gray-700">
              {{ amortData.debt.name }} · {{ amortData.debt.creditor }}
            </p>
            <p class="text-xs text-gray-400">
              TEA {{ amortData.debt.interestRate }}% · {{ amortData.rows.length }} cuotas totales
            </p>
          </div>
            ...
            <table class="min-w-full text-xs">
              ...
              <tbody class="divide-y divide-gray-50">
                ...
                >
                  ...
                  <td class="px-3 py-2 text-right font-medium text-emerald-700">
                  ...
                  <td
                    ...
                    :class="row.remainingBalance > 0 ? 'text-red-600' : 'text-emerald-600'"
                  >
                  ...
                  <td class="px-3 py-2 text-center">
                    <span
                      ...
                      :class="
                        statusConfig[row.status as keyof typeof statusConfig]?.cls ??
      ...
      >
        ...
        <p class="mt-4 text-sm font-medium text-gray-600">
          Selecciona una deuda para ver su tabla de amortización
        </p>
      </div>
    </div>
    ...
    <div v-else-if="activeTab === 'forecast'" class="space-y-5">
      <!-- Controls + summary -->
      <div
        class="rounded-2xl bg-gradient-to-br from-slate-700 to-slate-900 p-5 text-white shadow-lg"
      >
        <!-- Month selector -->
        <div class="mb-4 flex items-center justify-between">
          <div>
            <p class="text-xs font-semibold uppercase tracking-widest text-slate-300">
          ...
          <div class="flex gap-1.5 rounded-xl bg-white/10 p-1">
            <button
              ...
              :class="
                forecastMonths === m ? 'bg-white text-slate-800' : 'text-slate-300 hover:text-white'
              "
            >
              {{ m }}M
            </button>
        ...
        <div v-else-if="forecast" class="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div class="rounded-xl bg-white/10 p-3">
            <p class="text-[10px] font-semibold uppercase tracking-wide text-slate-300">
              Balance actual
            </p>
            <p
              class="mt-0.5 text-lg font-bold"
              :class="(forecast.currentBalance ?? 0) >= 0 ? 'text-white' : 'text-red-300'"
            >
          ...
          <div class="rounded-xl bg-white/10 p-3">
            ...
            <p class="mt-0.5 text-lg font-bold text-emerald-300">
          ...
          <div class="rounded-xl bg-white/10 p-3">
            ...
            <p class="mt-0.5 text-lg font-bold text-rose-300">
              {{ formatCurrency(forecastTotalDebt) }}
            </p>
          </div>
          <div class="rounded-xl bg-white/10 p-3">
            <p class="text-[10px] font-semibold uppercase tracking-wide text-slate-300">
            ...
            <p
              class="mt-0.5 text-lg font-bold"
              :class="forecastNet >= 0 ? 'text-emerald-300' : 'text-red-300'"
            >
      ...
      <template v-if="forecast && !forecastLoading">
          ...
          >
            ...
            >
              <div>
                <p
                  ...
                  :class="m.isCurrentMonth ? 'text-indigo-100' : 'text-gray-500'"
                >
                  {{ m.isCurrentMonth ? 'Mes actual' : m.yearMonth }}
                </p>
                <p
                  class="text-sm font-bold"
                  :class="m.isCurrentMonth ? 'text-white' : 'text-gray-800'"
                >
                  {{ m.fullLabel }}
                </p>
              </div>
              <div
                ...
                :class="
                  m.netBalance >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
                "
              >
            ...
            <div class="space-y-1.5 px-4 py-3 text-xs">
              ...
              <div class="flex justify-between border-t border-gray-100 pt-1.5 font-semibold">
                ...
                <span :class="m.runningBalance >= 0 ? 'text-gray-800' : 'font-bold text-red-600'">
        ...
        <!-- Debts freedom table -->
        <div
          v-if="forecast.debtsFreed.length > 0"
        ...
        >
          <div class="border-b border-gray-50 px-4 py-3">
            ...
            <p class="text-xs text-gray-400">
              Cada deuda pagada libera ese monto para ahorro o inversión
            </p>
          </div>
            ...
            >
              ...
              <div class="text-right">
                <p class="text-sm font-bold text-emerald-700">
                  +{{ formatCurrency(freed.monthlyRelief) }}/mes
                </p>
        ...
        <p class="text-center text-xs text-gray-400">
          * El forecast usa ingresos y gastos recurrentes registrados. Las tarjetas de crédito se
          estiman con el promedio de los últimos 3 meses. Los montos reales pueden variar.
        </p>
      </template>
    </div>
  </div>
