...
const incomeCount = computed(
  () => (categories.value ?? []).filter((c) => c.type === 'income').length
)
const expenseCount = computed(
  () => (categories.value ?? []).filter((c) => c.type === 'expense').length
)
...
<template>
  <div class="mx-auto max-w-5xl space-y-5">
    ...
    <div v-if="pending" class="flex items-center justify-center py-16">
      <div
        class="border-3 h-8 w-8 animate-spin rounded-full border-indigo-500 border-t-transparent"
      ></div>
    </div>
    ...
    <div v-else class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        ...
        <div
          ...
          :style="
            category.color ? `background-color: ${category.color}20; color: ${category.color}` : ''
          "
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-1 flex items-center gap-2">
            ...
            ></div>
            <span v-if="category.color" class="text-[10px] text-gray-400">{{
              category.color
            }}</span>
          </div>
        </div>
